...
public class Graphics implements
	...
	public void drawChars(char[] data, int offset, int length, int x, int y, int anchor) {
		if (data != null && length > 0) {
			...
			javax.microedition.util.StringEditorManager.getInstance().recordString(text);
			String replaced = javax.microedition.util.StringEditorManager.getInstance().replaceString(text);
			if (!replaced.equals(text)) {
				drawString(replaced, x, y, anchor);
				return;
			}
		}
	...
	public void drawString(String text, int x, int y, int anchor) {
		if (text != null && !text.isEmpty()) {
			javax.microedition.util.StringEditorManager.getInstance().recordString(text);
			text = javax.microedition.util.StringEditorManager.getInstance().replaceString(text);
		}
	...
	public void drawSubstring(String str, int offset, int len, int x, int y, int anchor) {
		if (str != null && len > 0) {
			...
			javax.microedition.util.StringEditorManager.getInstance().recordString(sub);
			String replaced = javax.microedition.util.StringEditorManager.getInstance().replaceString(sub);
			if (!replaced.equals(sub)) {
				drawString(replaced, x, y, anchor);
				return;
			}
		}
