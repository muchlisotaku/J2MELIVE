...
public class MicroActivity extends AppCompatActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		javax.microedition.util.StringEditorManager.getInstance().init(new File(appPath));
		javax.microedition.util.StringEditorManager.getInstance().addListener(() -> {
			Displayable cur = current;
	...
	private void showGameSpeedDialog() {
		final String[] speeds = {"0.25x (Slowest)", "0.5x (Slow-Mo)", "1.0x (Normal)", "1.5x (Fast)", "2.0x (2x Speed)", "3.0x (3x Speed)", "5.0x (Turbo)"};
		final float[] multipliers = {0.25f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 5.0f};
		float currentSpeed = Canvas.getSpeedMultiplier();
		int selected = 2;
		for (int i = 0; i < multipliers.length; i++) {
			if (Math.abs(multipliers[i] - currentSpeed) < 0.05f) {
				selected = i;
				break;
			}
		}
		...
		new AlertDialog.Builder(this)
				.setTitle(R.string.action_game_speed)
				.setSingleChoiceItems(speeds, selected, (dialog, which) -> {
					Canvas.setSpeedMultiplier(multipliers[which]);
